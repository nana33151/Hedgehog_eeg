...
def labels_to_matrices(labels, tgt_vocab_size, seq_len):
    ...
    for i in coordinates:
        for j in range(i+input_lag, i+DUR+input_lag//2):
            labels_matrices[j][labels[i]-1] = 1.0
            local_gistogram[labels[i]-1] += 1
            labels_matrices[j][tgt_vocab_size-1] = 0.0
    local_gistogram[4] = seq_len - len(coordinates)*DUR
...
with open("loss.txt", "w+") as file2:
    ...
    best_loss = max(best_loss,max(loss_list))
for j in range(EPOCHS):
    ...
    for i in range(len(training_datasets)):
        ...
        if i % 1000 == 999:
            ...
            running_loss = 0
            running_acc = 0
    with open("results.txt") as file1:
